...
namespace Spark
{
	...
	namespace Utils
	{
		static GLenum ToOpenGLTextureFromat(TextureFormat format)
		{
			...
			SK_CORE_ASSERT(false, "Unkonw texture format!");
			return 0;
		}
	}
		...
		: m_Width(0), m_Height(0), m_FilePath(path), m_Wrap(TextureWrap::Clamp)
	{
		...
		{
			SK_CORE_INFO("Loading HDR texture {0}, srgb={1}", path, srgb);
			m_ImageData.Data = (byte*)stbi_loadf(path.c_str(), &width, &height, &channels, 0);
			if (m_ImageData.Data == nullptr)
			{
				SK_CORE_ERROR("Failed to load HDR texture {0}, srgb={1}", path, srgb);
				return;
			}
			m_IsHDR = true;
			m_TextureFormat = TextureFormat::RGBA16F;
		}else
		{
		...
		RenderCommandQueue::Submit([this,srgb]()
		{
				...
				else
				{
					...
					GLenum internalFormat = Utils::ToOpenGLTextureFromat(m_TextureFormat);
					GLenum format = srgb ? GL_SRGB8 : (m_IsHDR ? GL_RGB : internalFormat);
		...
		,m_Format(format)
	{
		...
		RenderCommandQueue::Submit([=]()
		{
			...
			glTextureStorage2D(m_RendererID, levels, Utils::ToOpenGLTextureFromat(m_Format), width, height);
